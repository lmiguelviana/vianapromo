...
<div class="flex items-center justify-between mb-4">
    ...
    <div class="flex gap-2">
        ...
        <button type="button" onclick="rodarBotShopee(this)"
           class="flex items-center gap-2 bg-orange-600 text-white hover:bg-orange-700 px-4 py-2 rounded-lg text-sm font-medium transition">
            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
...
function appendStatus(txt) {
    pre.innerHTML += '\n<span style="color:#fb923c">' + txt.replace(/[&<>]/g, s => ({'&':'&amp;','<':'&lt;','>':'&gt;'}[s])) + '</span>';
    scrollBottom();
}
...
function liberarBotShopee(btn) {
    ...
    fetch(BASE + '/api/bot_lock_clear.php', {
        ...
        btn.disabled = false;
        appendStatus('Erro de rede ao liberar Bot Shopee.');
    });
}

function rodarBotShopee(btn) {
    btn.disabled = true;
    appendStatus('Disparando Bot Shopee...');
    fetch(BASE + '/api/bot_run.php', {
        ...
        appendStatus('Erro de rede ao rodar Bot Shopee.');
        btn.disabled = false;
    });
}
</script>

...
$logFile = __DIR__ . '/storage/bot_ml.log';

if (($_GET['action'] ?? '') === 'clear') {
    file_put_contents($logFile, '');
    header('Location: ' . BASE . '/logs-ml');
    exit;
}

function lerLog(string $path): array {
    if (!file_exists($path)) {
        return ['conteudo' => "Nenhum log encontrado.\nClique em \"Bot ML\" na Fila para iniciar.", 'tamanho' => 0];
    }
...
layoutStart('logs_ml', 'Logs Bot ML');
?>
...
<div class="flex items-center justify-between mb-4">
    <p class="text-sm text-gray-500">
        Bot ML — Mercado Livre + Magazine Luiza —
        <span class="font-mono text-xs text-gray-400">storage/bot_ml.log</span>
    ...
    <div class="flex gap-2">
        <button type="button" onclick="liberarBotML(this)"
        ...
        <a href="<?= BASE ?>/logs-ml?action=clear"
           onclick="return confirm('Limpar logs do Bot ML?')"
...
<div id="log-terminal">
    <div id="log-bar">
        ...
        <span style="font-size:11px; color:#64748b; margin-left:8px;">🤖 viana/storage/bot_ml.log</span>
    </div>
    <div id="log-body">
...
setInterval(() => {
    fetch(BASE + '/api/log_tail.php?bot=ml')
        .then(r => r.json())
        .then(d => {
            if (d.size !== lastSize) {
                lastSize = d.size;
